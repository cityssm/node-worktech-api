...
export class WorkTechAPI {
  ...
  constructor(mssqlConfig: mssql.config) {
    ...
    this.#mssqlConfig.options.connectTimeout = Math.max(
      this.#mssqlConfig.options.connectTimeout ?? 0,
      timeoutMillis
    )

    this.#mssqlConfig.options.requestTimeout = Math.max(
      this.#mssqlConfig.options.requestTimeout ?? 0,
      timeoutMillis
    )
  }
