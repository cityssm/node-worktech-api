 ...
 */
export async function updateWorkOrderResource(mssqlConfig, workOrderResource) {
    /*
     * Do the transaction
     */
    ...
    try {
      ...
      set SRISysID = @serviceRequestItemSystemId`;
        let request = transaction
            .request()
            .input('serviceRequestItemSystemId', workOrderResource.serviceRequestItemSystemId);
        /*
         * Update Work Description
         */
        if (workOrderResource.workDescription !== undefined) {
            request = request.input('workDescription', workOrderResource.workDescription);
            sql += ', WORKDESC = @workDescription';
        }
        /*
         * Update Work Order Number
         */
        if (workOrderResource.serviceRequestSystemId !== undefined &&
            workOrderResource.workOrderNumber !== undefined) {
            request = request
                .input('serviceRequestSystemId', workOrderResource.serviceRequestSystemId)
         ...
         */
        if (workOrderResource.quantity !== undefined &&
